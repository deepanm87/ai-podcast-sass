...
export const retryJobFunction = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    if (!transcript.text || transcript.text.length === 0) {
      throw new Error("Cannot generate content: transcript text is empty. Please re-upload the file.")
    }

    const jobsRequiringChapters = ["keyMoments", "youtubeTimestamps"]
    if (jobsRequiringChapters.includes(job)) {
      if (!transcript.chapters || transcript.chapters.length === 0) {
        throw new Error(`Cannot generate ${job}: transcript has no chapters. This podcast may be too short or lack distinct topics for chapter detection.`)
      }
    }
