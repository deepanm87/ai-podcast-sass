...
import { openai } from "@/inngest/lib/openai-client"
...
export async function generateYouTubeTimestamps(
  ...
  const response = (await step.ai.wrap(
    ...
    createCompletion,
    {
      ...
      messages: [
        {
          role: "system",
          content: "You are a YouTube content expert who creates SHORT, DESCRIPTIVE TITLES for video chapters. CRITICAL: You create TITLES (like 'Introduction to AI'), NOT transcript text or full sentences. Always respond with valid JSON."
        },
        {
          role: "user",
          content: prompt
  ...
  const aiTimestamps = chapterData.map(chapter => {
    ...
    if (!aiTitle) {
      console.warn(`No AI title found for chapter ${chapter.index}, using fallback: "${chapter.headline}"`)
      }
